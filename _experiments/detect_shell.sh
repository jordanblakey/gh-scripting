...
if [ -n "$BASH_VERSION" ]; then
  ...
  if [ "$_final_candidate_name" = "bash" ]; then
    _current_shell="bash"; _add_detail "Identified as bash by name"
  elif [ "$_final_candidate_name" = "zsh" ]; then
    _current_shell="zsh";  _add_detail "Identified as zsh by name"
  elif [ "$_final_candidate_name" = "ksh" ] || [ "$_final_candidate_name" = "mksh" ] || [ "$_final_candidate_name" = "pdksh" ]; then
    _current_shell="ksh"; _add_detail "Identified as ksh by name ($_final_candidate_name)"
  elif [ "$_final_candidate_name" = "dash" ]; then
    _current_shell="dash"; _add_detail "Identified as dash by name"
  elif [ "$_final_candidate_name" = "fish" ]; then
    _current_shell="fish"; _add_detail "Identified as fish by name"
  elif [ "$_final_candidate_name" = "sh" ]; then
    _current_shell="sh (generic or compatible)"; _add_detail "Identified as sh by name"
